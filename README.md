...
This project is a proof of concept for processing natural language text queries. The system validates and processes queries that include Boolean operators (AND, OR, ANDNOT), parentheses, and quoted strings. The primary goal is to validate the structure of the queries and generate a human-readable representation of the query's logical structure.

## Features

- **Validation**: Ensures that queries have balanced parentheses and quotes, and that Boolean operators are used correctly.
...
3. **Boolean Operator Validation**: Confirms that Boolean operators are correctly used and surrounded by valid expressions.
...
```plaintext
Quoted String: "A AND (B)"
AND
Start Group:
  B
  OR
  Quoted String: "C AND D"
End Group
```
...
```csharp
string query = ""A AND (B)" AND (B OR "C AND D")";
